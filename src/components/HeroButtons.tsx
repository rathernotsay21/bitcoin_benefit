...
export function HeroButtons() {
  return (
    <div className="mt-12 flex flex-col sm:flex-row justify-center gap-6 px-4">
      <Link 
        href="/calculator" 
        className="hero-btn-primary inline-flex items-center justify-center gap-3 px-10 py-4 font-bold text-lg rounded-xl transition-all duration-300 group border-2 border-bitcoin bg-slate-900/90 hover:bg-slate-800 hover:border-yellow-400 shadow-lg hover:shadow-xl hover:shadow-bitcoin/25"
        style={{ color: '#F7931A' }}
      ...
      <Link 
        href="/historical" 
        className="hero-btn-secondary inline-flex items-center justify-center gap-3 px-10 py-4 font-bold text-lg rounded-xl transition-all duration-300 group border-2 border-slate-600 bg-slate-800/60 backdrop-blur-sm hover:bg-slate-700/60 hover:border-blue-400 shadow-lg hover:shadow-xl hover:shadow-blue-500/25"
        style={{ color: '#e2e8f0' }}
        onMouseEnter={(e) => e.currentTarget.style.color = '#ffffff'}
